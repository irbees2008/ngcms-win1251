...
<h3 id="reply-title" class="comment-reply-title"><a href="?mod=news"> {{ lang.editnews['news_title'] }}</a></h3>
...
<td  nowrap>{{ lang.editnews['postid_short'] }}</td>
<td   nowrap>{{ lang.editnews['date'] }}</td>
<td >&nbsp;</td>
<td  >{{ lang.editnews['title'] }}</td>
{% if flags.comments %}<td  style=" text-align:right;"><i class="fa fa-comments-o"></i></td>{% endif %}
<td  style=" text-align:right;"><i class="fa fa-eye"></i></td>
<td >{{ lang.editnews['category'] }}</td>
<td >{{ lang.editnews['author'] }}</td>
<td >&nbsp;</td>
<td ><input class="check" type="checkbox" name="master_box" title="{{ lang.editnews['select_all'] }}" onclick="javascript:check_uncheck_all(editnews)" /></td>
</tr>
{% for entry in entries %}
<tr align="left" >
	<td  class="contentEntry1">{{ entry.newsid }}</td>
	<td  class="contentEntry1">{{ entry.itemdate }}</td>
	<td  class="contentEntry1" cellspacing=0 cellpadding=0 style="padding:0; margin:0;" nowrap>
		{% if entry.flags.mainpage %}<i class="fa fa-home" title="Main"></i> {% endif %}
		{% if (entry.attach_count > 0) %}<i class="fa fa-paperclip" title="{{ lang['attach.count'] }}: {{ entry.attach_count }}"></i>{% endif %}
		{% if (entry.images_count > 0) %}<i class="fa fa-file-image-o" title="{{ lang['images.count'] }}: {{ entry.images_count }}"></i> {% endif %}
	</td>
	...
	<td class="contentEntry1">{% if (entry.state == 1) %}<i class="fa fa-check" title="{{ lang['state.published'] }}"></i> {% elseif (entry.state == 0) %}<i class="fa fa-times" title="{{ lang['state.unpiblished'] }}"></i> {% else %}<i class="fa fa-circle-o" title="{{ lang['state.draft'] }}"></i> {% endif %} </td>
