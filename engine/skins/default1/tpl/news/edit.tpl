...
<div  class="comments-area">
	...
	<a href="?mod=news">{{ lang.editnews['news_title'] }}</a> &#8594; {{ lang.editnews['editnews_title'] }} "<a href="?mod=news&action=edit&id={{ id }}">{{ title }}</a>" ({% if (approve == -1) %}{{ lang['state.draft'] }}{% elseif (approve == 0) %}{{ lang['state.unpublished'] }}{% else %}{{ lang['state.published'] }} &#8594; <small><a href="{{ link }}" target="_blank">{{ link }}</a></small>{% endif %})
...
{% if (pluginIsActive('comments')) %}<input type="button" onmousedown="javascript:ChangeOption('comments')" value="{{ lang.editnews['bar.comments'] }} ({{ plugin.comments.count }})" class="navbutton" />{% endif %}
</td>
</tr>
<tr><td>
...
<tr>
<td class="contentHead">{{ lang.editnews['date.manage'] }}</td>
</tr>
<tr>
...
<thead>
<tr class="contHead"><td>ID</td><td width="80">{{ lang.editnews['attach.date'] }}</td><td width="10">&nbsp;</td><td>{{ lang.editnews['attach.filename'] }}</td><td width="90">{{ lang.editnews['attach.size'] }}</td><td width="40">DEL</td></tr>
</thead>
<tbody>
...
<tr>
 <td>{{ entry.id }}</td>
 <td>{{ entry.date }}</td>
 <td><a style="cursor:pointer" onclick="ChangeOption('maincontent'); insertext('[attach#{{ entry.id }}]{{ entry.orig_name }}[/attach]','', currentInputAreaID)" title='{{ lang['tags.file'] }}'><img src="{{ skins_url }}/tags/attach.png" width="16" height="16" border="0" /></a></td>
...
<tr><td colspan="3">&nbsp;</td><td colspan="2"><input type="button" class="button" value="{{ lang.editnews['attach.more_rows'] }}" style="width: 100%;" onclick="attachAddRow();" /></td></tr>
 ...
 <table width="100%" cellspacing="0" cellpadding="0" border="0" id="rightBar">
  <tr><td><span class="f15">{{ lang.editnews['editor.comminfo'] }}</span>
  <div class="list">
...
{{ lang['editor.dedit'] }}: <b>{{ editdate }}</b>
   ...
   <td><span class="f15">{{ lang.editnews['category'] }}</span><div class="list">{{ mastercat }} {% if (flags.mondatory_cat) %}&nbsp; <span style="font-size: 16px; color: red;"><b>*</b></span>{% endif %}</div></td>
  </tr>
   <tr>
   <td><span class="f15">{{ lang['editor.configuration'] }}</span>
  <div class="list">
   ...
   {% endif %}<br />
    ...
    <td><span class="f15">{{ lang['editor.extcat'] }}</span>
