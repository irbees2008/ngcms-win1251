...
<tr>
<td width=100% colspan="5" class="contentHead"><img src="{{ skins_url }}/images/nav.gif" hspace="8"><a href="?mod=static">{{ lang['static_title'] }}</a></td>
</tr>
</table>
...
<tr>
<td width="100%" align="right">{{ lang['per_page'] }} <form action="{{ php_self }}" method="get" name="options_bar"><input type="hidden" name="mod" value="static" /><input style="text-align: center" name="per_page" value="{{ per_page }}" type="text" size="3" /> <input type="submit" value="{{ lang['do_show'] }}" class="button" /></form> &nbsp;</td>
</tr>
</table>
...
<tr align="left">
<td class="contentEntry1">{% if (perm.modify) %}<input name="selected[]" value="{{ entry.id }}" class="check" type="checkbox" />{% endif %}</td>
<td class="contentEntry1"><div style="margin-right: 5px;">{{ entry.status }}</div></td>
...
<td width="50%" class="contentEdit" align="right">{% if (perm.modify) %}<input type="button" value="{{ lang['addstatic'] }}" onclick="document.location='?mod=static&action=addForm'; return false;" class="button" />{% endif %} &nbsp;</td>
</tr>
