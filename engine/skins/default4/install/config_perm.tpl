...
<div class="permData">
<table width="100%">
<thead><tr><td>{l_perm.php.param}</td><td>{l_perm.php.recommended}</td><td>{l_perm.php.installed}</td></thead>
...
<div class="permData">
<table width="100%">
<thead><tr><td>{l_perm.file}</td><td>CHMOD</td><td>{l_perm.status}</td></thead>
{chmod}
</table>
</div>
</div>
...
<br/>
<table width="100%">
<tr><td width="33%"><input type="button" value="&laquo;&laquo; {l_button.back}" onclick="document.getElementById('stage').value='0'; form.submit();" class="filterbutton"/></td><td align="center">[error_button]<input style="background-color: red; color: white; font-weight: bold;" type="button" value="Повторить проверку" onclick="document.getElementById('stage').value='1'; form.submit();" class="filterbutton"/>[/error_button]</td><td width="33%" style="text-align: right;"><input type="submit" value="{l_button.next} &raquo;&raquo;" class="filterbutton"/></td></tr>
</table>
</form>
</div>