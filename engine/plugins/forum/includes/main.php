...
function recent_events_forum() {
	...
	foreach ($mysql->select('SELECT * FROM ' . prefix . '_users ORDER BY reg DESC LIMIT 10') as $row) {
		switch ($row['status']) {
			case 1:
				$color_start = '<span style="color:' . $GROUP_PERM[$row['status']]['group_color'] . ';">';
				$color_end = '</span>';
				break;
			case 2:
				$color_start = '<span style="color:' . $GROUP_PERM[$row['status']]['group_color'] . ';">';
		...
		foreach ($online as $row) {
			if ($row['last_time'] > $last_time) {
				if (isset($row['users_status']) && $row['users_status'] == 0) {
					...
					$color_start = '<span style="color:' . $GROUP_PERM[$row['users_status']]['group_color'] . ';">';
					$color_end = '</span>';
